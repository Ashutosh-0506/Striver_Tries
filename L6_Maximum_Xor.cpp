...
// Function to find the maximum XOR between any two numbers in two given arrays
int maxXOR(int n, int m, vector<int> &arr1, vector<int> &arr2)
{
    int maxx = 0; // Initialize the maximum XOR result
    Trie trie;    // Create a Trie data structure
    // Step 1: Insert all elements from the first array into the Trie
    for (auto &it : arr1)
    {
        trie.insert(it);
    }
    // Step 2: Find the maximum XOR with any number from the second array
    for (auto &it : arr2)
    {
        maxx = max(maxx, trie.maxXor(it)); // Update the maximum XOR result
    }
    return maxx; // Return the maximum XOR result
}
